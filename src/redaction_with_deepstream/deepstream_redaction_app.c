...
GstElement *source = NULL, *decoder = NULL, *streammux = NULL,
    *queue_pgie = NULL, *pgie = NULL, *sgie = NULL, *nvvidconv_osd =
    ...
    gpointer u_data)
{
  ...
  NvDsObjectMeta * l_user_meta = NULL;
  ...
  for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
    ...
    for (l_obj = frame_meta->obj_meta_list; l_obj != NULL;
      ...
      // Only process gie-unique-id = 2 (license plates)
      if (obj_meta->unique_component_id != 2){
        continue;
      }

      /* Draw skin-color patch to cover license plates (class_id = 0) */
...
main (int argc, char *argv[])
{
  ...
  GOptionContext *ctx;
  GError *err = NULL;
  // TODO: Add a second entry for the secondary gie
  GOptionEntry entries[] = {
    {"pgie_config", 'c', 0, G_OPTION_ARG_STRING, &pgie_config,
          "(required) configuration file for the nvinfer detector (primary gie)",
        NULL},
    {"sgie_config", 's', 0, G_OPTION_ARG_STRING, &sgie_config,
          "(required) configuration file for the nvinfer detector (secondary gie)",
        NULL},
...
contact: Shuo Wang (shuow@nvidia.com)");
  ...
  /* Check input arguments */
  if (!sgie_config) {
    g_printerr ("missing sgie configuration file\n");
    g_printerr ("run: %s --help for usage instruction\n", argv[0]);
    return -1;
  }
  ...
  // We need to create a Secondary Object Detector for License Plates
  sgie = gst_element_factory_make ("nvinfer", "secondary-nvinference-engine");
  g_object_set (G_OBJECT (sgie), "config-file-path", sgie_config, NULL);
  ...
  if (!pipeline || !source || !video_full_processing_bin
      || !queue_pgie || !pgie || !sgie || !nvvidconv_osd
      || !osd || !sink) {
  ...
  gst_bin_add_many (GST_BIN (video_full_processing_bin), streammux,
      queue_pgie, pgie, sgie, nvvidconv_osd, osd, NULL);
  /* link the elements together */
  gst_element_link_many (streammux, queue_pgie, pgie, sgie, nvvidconv_osd, osd,
      NULL);
